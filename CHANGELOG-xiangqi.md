...
## 2026-09-27（日）：围棋 3D 全面重设计 + 2D 渲染修复（多轮迭代）

### 3D 棋子与棋盘重设计（参考用户提供的真实围棋照片）
- **棋子**：LatheGeometry 椭圆剖面，加厚至真实云子比例（中心高 0.42R / 边缘 0.15R，高宽比≈1:4.8，告别"纸片感"）
- **材质**：黑子极黑哑光（0x050505 / roughness 0.55 / clearcoat 0.08，俯视纯黑不泛灰）；白子乳白瓷光（0xfaf6ec / clearcoat 0.3）
- **光影**：主光 0.8、环境光 0.2、半球光 0.22、环境贴图反射 0.12（俯视黑白分明的决定性调整）；ACES exposure 1.0
- **棋盘**：木纹加深 #c29155 衬托白子、网格线深棕黑、接触阴影 0.26 增强分隔
- **视角控制**（参考中国象棋 3D 方案）：左键拖拽旋转、右键拖拽平移、滚轮缩放、单指旋转/双指缩放/三指平移、右上角「重置视角」按钮；拖拽与点击落子智能区分（mouseup 距离兜底拦截）
- **环境贴图**：程序化摄影棚柔光（CubeTexture 6 面渐变）

### 2D 棋盘渲染修复
- **根因**：SVG `<circle cx="50%">` 百分比相对 viewBox 而非已 translate 的 group → 所有棋子堆叠在棋盘中心
- 修复：棋子/标记 cx/cy → 0（跟随交叉点 group）；棋子半径按格宽自适应（0.46 格）
- 白子加深色轮廓（#5a5548 描边）与木色棋盘区分；星位/领地/提示环尺寸自适应

### 验证
- tsc --noEmit、vite build 通过；浏览器实测：3D 俯视黑白分明（黑子纯黑/白子乳白）、视角旋转/平移/缩放/重置可用、落子不误触发；2D 棋子按交叉点分布、坐标换算正确
